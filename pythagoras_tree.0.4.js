...
//Don't know what this is for. An internal representation?
function ColorRamp(){}


//drawing PySquare is referenced from bottom-left corner,
//but
//rotating PySquare is referenced from top-left corner
//
//this is confusing and needs to be remedied.
//
function PySquare(svgCanvas, upperLeftCoordinate, size, degreeRotation, colorInHex){

 //svg.js stuff
 this.canvas = svgCanvas, 
 this.size = size, //in pixels
 this.rotation = degreeRotation, //rotation is counter-clockwise
 ...
 /*
  * After rotating the PySquare, I re-name the coordinates to match an orientation where the "bottom" corners are the corners touching the lower/base PySquare.
  *
  */
...
/*
 * A recursive function that takes an "initial" PySquare as input, and draws a pythagoras tree using that initial square as the base of the tree.
 *
 * The recursive function uses the base square, and draws two successive squares: the left and right branches, then is called again using each of the left and right branch squares as the base of a "new tree".
 *
 * Fill colors for the PySquares are chosen as a function of the iteration depth.
 *
 */
